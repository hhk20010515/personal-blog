  ...
  callbacks: {
    async session({ session, user }) {
      if (session.user) {
        ...
        try {
          ...
          console.log('Database error in session callback:', error)
    ...
    async signIn({ user, account, profile, email, credentials }) {
      ...
      try {
        ...
        console.log('Database error in signIn callback:', error)
        // Allow sign in if database is unavailable during build
      }
  ...
  events: {
    async createUser({ user }) {
      try {
        ...
        console.log('Database error in createUser event:', error)
      }
    },
  },
  session: {
